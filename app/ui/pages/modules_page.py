    ...
    def delete_module(self):
        ...
        if msg_box.exec() == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(self, "Успех", f"Модуль '{module['title']}' успешно удален")

            except Exception as e:
                QMessageBox.critical(
                    self, "Ошибка", f"Не удалось удалить модуль: {str(e)}")
                ...
                try:
                    ...
                    for lesson in lessons:
                        if lesson['title'].lower() == title.lower():
                            QMessageBox.warning(self, "Ошибка", f"Урок с названием '{title}' уже существует в этом модуле")
                            return

                    # Проверяем задачи
                    for task in tasks:
                        if task['title'].lower() == title.lower():
                            QMessageBox.warning(self, "Ошибка", f"Задача с названием '{title}' уже существует в этом модуле" )
                            return
                    ...
                    if course and course['github_path']:
                        ...
                        if module.get('site_id'):
                            ...
                            QMessageBox.information(self, "Успех", f"Урок '{title}' успешно создан в модуле '{module['title']}'")
                        else:

...
class GushubAPI:
    ...
    def _make_request(self, method: str, endpoint: str, data: Optional[Dict] = None, retry_auth: bool = True) -> Dict:
        ...
        try:
            ...
            # Если получили 401 и разрешена повторная авторизация
            if response.status_code == 401 and retry_auth:
                ...
                if username and password:
                    ...
                    self.login(username, password)
                    # Повторяем запрос с новыми куками, но без повторной авторизации
                    return self._make_request(method, url, data, retry_auth=False)
    ...
    def login(self, username: str, password: str) -> Dict:
        ...
        # Отправляем запрос на логин без повторной авторизации
        response = self._make_request('POST', '/api/auth/login', {'username': username, 'password': password}, retry_auth=False)
